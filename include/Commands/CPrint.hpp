...
private:
	...
	//Returns the path if dir was found or it returns an empty string if not 
	std::filesystem::path DirecoryExists(const std::string dirName, const std::filesystem::path& executePath) const;
};
